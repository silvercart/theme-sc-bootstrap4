	...
	$.fn.extend({
		slidorion: function(options) {
			...
			return this.each(function() {
				...
				sliderCount = $('.silvercart-slidorion-slider > div', obj).length;
				obj.data('slideCount', sliderCount);
				
				accordionCount = $('.silvercart-slidorion-accordeon > .silvercart-slidorion-link-header', obj).length;
				...
				if(sliderCount==accordionCount){
					...
					$('.silvercart-widget-slidorion-productgroup-slider > div:eq('+(current-1)+')', obj).css('z-index',zPos);
					zPos++;
					
					if(effect != "fade" || effect != "none"){
						$('.silvercart-slidorion-slider > div', obj).css({'top':'0','left':'-600px'});
						$('.silvercart-slidorion-slider > div:eq('+(current-1)+')', obj).css({'top':'0','left':'0'});
					}
					
					$('.silvercart-slidorion-link-content', obj).hide();
					$('.silvercart-slidorion-accordeon .silvercart-slidorion-link-header:eq('+(current-1)+')', obj)
						.addClass('active')
						.next()
						.show();
					
					$(".silvercart-slidorion-link-header", obj).click(sectionClicked);					
				...
				function animation(current, section, effect, speed, easingOption){
					if(!active && loaded){
						...
						$current = $('.silvercart-slidorion-slider > div:eq('+(current-1)+')', obj);
						$new = $('.silvercart-slidorion-slider > div:eq('+(section-1)+')', obj);
				...
				function sectionClicked(){
					if(active == false) {
						...
						if(section==current){
							...
							$('.silvercart-slidorion-link-header.active', obj)
								.removeClass('active')
								.next('.silvercart-slidorion-link-content')
								.slideUp();
				...
				function playSlider(current, effect, speed, easingOption){
					...
					$('.silvercart-slidorion-accordeon .silvercart-slidorion-link-header:eq('+nextSection+')', obj).trigger('click', sectionClicked);
				}
				...
				function resetLayers() {
					for(var i=sliderCount;i>0;i--){
						$('.silvercart-slidorion-slider > div:eq('+(i-1)+')', obj).css('z-index',zPos);
						zPos++;
					}
				}
