...
<% if $EnablePackstation %>
    <div class="row">
        <div class="span8 col-md-8">{$Fields.dataFieldByName(IsPackstation).FieldHolder}</div>
    </div>
<% end_if %>
...
<% if $EnablePackstation %>
    <div class="packstation-address-data">
        <div class="row">
            <div class="span4 col-sm-4">{$Fields.dataFieldByName(PostNumber).FieldHolder}</div>
            <div class="span4 col-sm-4">{$Fields.dataFieldByName(Packstation).FieldHolder}</div>
        </div>
    </div>
<% end_if %>
    <div class="row">
...
<% end_loop %>
    <a class="btn btn-secondary js-link" id="silvercart-add-address-form-cancel-id" href="{$CancelLink}" title="<%t SilverCart\Model\Pages\Page.CANCEL 'Cancel' %>"><%t SilverCart\Model\Pages\Page.CANCEL 'Cancel' %></a>
<% if $IncludeFormTag %>
</form>
<% end_if %>
