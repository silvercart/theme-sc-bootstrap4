...
<% end_if %>
    ...
    <div class="row">
        ...
        <div class="span4 col-sm-4">{$Fields.dataFieldByName(AcademicTitle).FieldHolder}</div>
    </div>
    <div class="row">
...
<% if $demandBirthdayDate %>
    ...
    <div class="row">
        <div class="span4 col-sm-4">{$Fields.dataFieldByName(BirthdayDay).FieldHolder}</div>
        <div class="span4 col-sm-4">{$Fields.dataFieldByName(BirthdayMonth).FieldHolder}</div>
        <div class="span4 col-sm-4">{$Fields.dataFieldByName(BirthdayYear).FieldHolder}</div>
    </div>
<% end_if %>
    <h4><%t SilverCart\Model\Pages\Page.ACCESS_CREDENTIALS 'Access Credentials' %></h4>
    <div class="row">
        <div class="span4 col-sm">{$Fields.dataFieldByName(Email).FieldHolder}</div>
        <div class="span4 col-sm">{$Fields.dataFieldByName(EmailCheck).FieldHolder}</div>
    </div>
    <div class="row">
        <div class="span4 col-sm">{$Fields.dataFieldByName(Password).FieldHolder}</div>
        <div class="span4 col-sm">{$Fields.dataFieldByName(PasswordCheck).FieldHolder}</div>
    </div>
    ...
    <div class="clearfix">
    <% loop $Actions %>
        <button class="btn btn-sm btn-primary pull-right" type="submit" id="{$ID}" title="{$Title}" value="{$Value}" name="{$Name}">{$Title} <span class="fa fa-caret-right"></span></button>
    <% end_loop %>
    </div>
<% if $IncludeFormTag %>
</form>
