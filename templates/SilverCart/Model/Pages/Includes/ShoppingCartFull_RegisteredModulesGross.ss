    ...
    <% loop $registeredModules %>
        ...
        <% if $TaxableShoppingCartPositions %>
                <tr>
                    ...
                    <td class="text-right" data-title="<%t SilverCart\Model\Pages\Page.SUBTOTAL 'subtotal' %>"><strong><% with $CurrentMember.ShoppingCart %>{$TaxableAmountGrossWithoutFeesAndChargesAndModules.Nice}<% end_with %></strong></td>
                </tr>
            <% loop $TaxableShoppingCartPositions %>
                <tr>
                    ...
                    <td class="text-right">{$PriceTotalFormatted}
