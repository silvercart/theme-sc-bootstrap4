<div class="product-img-box card-block">
    <div class="product-img thumbnail text-center">
    <% if $ListImage %>
        <% with $ListImage %>
        <a class="fancybox" href="{$Link}" data-fancybox-group="silvercart-standard-product-image-group" title="{$Up.Title}"><img class="img-fluid img-responsive" src="{$Fit(940,450).URL}" alt="{$Up.Title}" /></a>
        <% end_with %>
    <% end_if %>
    </div>
    ...
    <div class="row">
        ...
        <% loop $getImages %>
            <% if not $First %>
            <a class="fancybox float-xs-left" href="{$Image.Link}" data-fancybox-group="silvercart-standard-product-image-group" title="{$Product.Title}"><img class="img-thumbnail" src="{$Image.Pad(100,80).URL}" alt="{$Product.Title}" /></a>
            <% end_if %>
        <% end_loop %>
        </div>
    </div>
...
<div class="card-block">
    ...
    <div class="span4 col-lg-6 bordered padding">
        <% if $PriceIsLowerThanMsr %>
        <span class="tag absolute top-left" title="<%t SilverCart\Model\Product\Product.Sale 'Sale' %>"><i class="tag-sale"><%t SilverCart\Model\Product\Product.Sale 'Sale' %>!</i></span>
        <% end_if %>
        <% if $isNewProduct %>
        <span class="tag absolute top-left" title="<%t SilverCart\Model\Product\Product.New 'New' %>"><i class="tag-new"><%t SilverCart\Model\Product\Product.New 'New' %>!</i></span>
        <% end_if %>
