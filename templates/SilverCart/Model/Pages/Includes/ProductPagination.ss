...
<nav class="text-center">
    ...
    <% with $getProducts %>
        ...
        <li class="page-item active"><a class="page-link highlight" href="javascript:;">{$PageNum}</a></li>
            <% else_if $Link %>
        <li class="page-item"><a href="{$Link}#scpgpct" class="page-link" title="<%t SilverCart\Model\Pages\Page.GOTO_PAGE 'go to page {count}' count=$PageNum %>">{$PageNum}</a></li>
            <% else %>
        <li class="page-item disabled"><a href="#" class="page-link">&hellip;</a></li>
            <% end_if %>
        <% end_loop %>
        <% if $NotLastPage %>
        <li class="page-item"><a href="{$NextLink}#scpgpct" rel="next" aria-label="<%t SilverCart\Model\Pages\Page.NEXT 'Next' %>" class="page-link" title="<%t SilverCart\Model\Pages\Page.NEXT 'Next' %>">
